...
import { Video, Sparkles, Play, FileText, Loader2 } from 'lucide-react'
      ...
      <header className="bg-black/30 backdrop-blur-sm border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white">X Video → Sora Script</h1>
              <p className="text-gray-400 text-sm">Scrape videos, transcribe, and generate Sora AI prompts</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 mb-8 border border-white/20">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-2xl font-bold text-white">Analyze X Videos</h2>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
                Number of Videos (saves API quota)
              </label>
              <input
                type="number"
                min="1"
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              <h3 className="text-xl font-bold text-white mb-4">
                📹 Scraped Videos from @{results.username}
              </h3>
                ...
                {results.scraped_videos.map((video, index) => (
                  <div key={video.id} className="bg-black/30 rounded-lg p-4 border border-white/10">
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-sm font-medium text-blue-400">Video {index + 1}</span>
                      <a 
                        href={video.tweet_url} 
                        target="_blank" 
                    ...
                    <p className="text-gray-300 text-sm mb-2">{video.text}</p>
